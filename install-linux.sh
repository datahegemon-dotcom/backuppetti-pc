#!/usr/bin/env bash
#
# BackupPetti Box — one-command installer for Linux.
#
...
#
# Run it with:
#   curl -fsSL https://datahegemon-dotcom.github.io/backuppetti-pc/install-linux.sh | sudo bash
#
set -euo pipefail
...
arch="$(uname -m)"
case "$arch" in
  aarch64|arm64) asset="backuppetti-agent-linux-arm64" ;;   # ARM Linux (64-bit)
...
if [ "$(id -u)" -ne 0 ]; then
  ...
  echo "  curl -fsSL https://datahegemon-dotcom.github.io/backuppetti-pc/install-linux.sh | sudo bash"
  exit 1
fi
...
if [ -z "${RUN_USER:-}" ] || ! id "$RUN_USER" >/dev/null 2>&1; then
  echo "❌ Could not work out your username (got '${RUN_USER:-}'). Re-run with: sudo bash install-linux.sh"
  exit 1
fi
...
echo "   Dashboard (on this PC): http://localhost:8088"
